...
export default function UseMemoTest01(params) {
    // useMemo는 재렌더링 사이에 계산 결과를 캐싱(기억 = 메모이제이션)할 수 있게 해주는 RecatHook
    ...
    // 계속 바뀌지 않는 계산 결과를 만들때 (무거운 계산(반복문, 정렬, 필더링 등))
    ...
    // 처음 렌더링할때만 heavyWork 실행하고, 값 저장(메모이제이션)

    // 의존성 배열이 비어 있어 최초 1번만 실행
    // const result = useMemo(()=> heavyWork(), []);
    
    // 의존성 배열이 필요한 경우 의존성 배열이 변경되면 랜더링 된다.
